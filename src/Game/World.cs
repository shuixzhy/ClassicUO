...
namespace ClassicUO.Game
{
    ...
    internal static class World
    {
        ...
        public static uint SearchObject(SCAN_TYPE_OBJECT scanType, SCAN_MODE_OBJECT scanMode)
        {
            ...
            int distance = ProfileManager.Current.MaxRange;
            ...
            else
            {
                if (scanMode == SCAN_MODE_OBJECT.SMO_NEAREST)
                {
                    foreach (Mobile mobile in Mobiles)
                    {
                        ...
                        else if (scanType == SCAN_TYPE_OBJECT.STO_SIMILARS)
                        {
                            if (!((mobile.IsRenamable || mobile.NotorietyFlag == World.Player.NotorietyFlag) &&
                        ...
                        else if (scanType == SCAN_TYPE_OBJECT.STO_HOSTILE)
                        {
                            if (mobile.NotorietyFlag == NotorietyFlag.Ally ||
                                ...
                                mobile.NotorietyFlag == NotorietyFlag.Invulnerable ||
                                mobile.Distance < ProfileManager.Current.MinRange)
                            {
                                continue;
                            }
                        }
                        else if (scanType == SCAN_TYPE_OBJECT.STO_MURDERER)
                        {
                            if (mobile.NotorietyFlag != NotorietyFlag.Murderer)
                            {
                                continue;
                            }
                        }
                ...
                else if (scanMode == SCAN_MODE_OBJECT.SMO_WEAK)
                {
                    foreach (Mobile mobile in Mobiles)
                    {
                        if (mobile.IsDestroyed || mobile == Player || mobile.Distance > ProfileManager.Current.MaxRange)
                            continue;
                        ...
                        int health = mobile.Hits;
                        if (health < minhits && health != 0 )
                            minhits = health;
                    }
                }
                foreach (Mobile mobile in Mobiles)
