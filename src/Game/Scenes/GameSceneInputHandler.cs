            ...
            if (canExecuteMacro)
            {
                ...
                if (macro != null && e.keysym.sym != SDL.SDL_Keycode.SDLK_UNKNOWN)
                {
                    ...
                    else
                    {
                        if (macro != null)
                        {
                            if (macro.Repeat == true)
                            {
                                if (macro == MacroManager.Last)
                                {
                                    MacroManager.Repeat = false;
                                    MacroManager.Last = null;
                                }
                                else
                                {
                                    MacroManager.Repeat = true;
                                    MacroManager.Delay = macro.Delay;
                                    Macros.SetMacroToExecute(macro.FirstNode);
                                    Macros.WaitForTargetTimer = 0;
                                    Macros.Update(true);
                                    MacroManager.Last = macro;
                                }
                            }
                            else
                            {
