...
namespace ClassicUO.Game.UI.Gumps
{
    internal class NameOverHeadHandlerGump : Gump
    {
        ...
        public NameOverHeadHandlerGump() : base(0, 0)
        {
            ...
            Add(all = new RadioButton(0, 0x00D0, 0x00D1, LanguageManager.Current.UI_NameOverHead_All, color: 0xFFFF)
            {
                IsChecked = NameOverHeadManager.TypeAllowed == NameOverheadTypeAllowed.All
            });

            Add(mobiles = new RadioButton(0, 0x00D0, 0x00D1, LanguageManager.Current.UI_NameOverHead_MobilesOnly, color: 0xFFFF)
            {
                Y = all.Y + all.Height,
                IsChecked = NameOverHeadManager.TypeAllowed == NameOverheadTypeAllowed.Mobiles
            });

            Add(items = new RadioButton(0, 0x00D0, 0x00D1, LanguageManager.Current.UI_NameOverHead_ItemsOnly, color: 0xFFFF)
            {
                Y = mobiles.Y + mobiles.Height,
                IsChecked = NameOverHeadManager.TypeAllowed == NameOverheadTypeAllowed.Items
            });

            Add(mobilesCorpses = new RadioButton(0, 0x00D0, 0x00D1, LanguageManager.Current.UI_NameOverHead_MobielAndCorpsesOnly, color: 0xFFFF)
            {
