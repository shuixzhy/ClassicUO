...
namespace ClassicUO.Game.UI.Gumps.CharCreation
{
    internal class CreateCharAppearanceGump : Gump
    {
        ...
        private void HandleGenreChange()
        {
            ...
            Add(_hairLabel = new Label(ClilocLoader.Instance.GetString(race == RaceType.GARGOYLE ? 1112309 : 3000121), true, 999, font: 0xFF)
            {
                X = 98, Y = 142
            }, 1);
            Add(_hairCombobox = new Combobox(97, 162, 120, content.Labels, CurrentOption[Layer.Hair]), 1);
            ...
            if (!isFemale && race != RaceType.ELF)
            {
                ...
                Add(_facialLabel = new Label(ClilocLoader.Instance.GetString(race == RaceType.GARGOYLE ? 1112511 : 3000122), true, 999, font: 0xFF)
                {
                    X = 98, Y = 202
                }, 1);
                Add(_facialCombobox = new Combobox(97, 222, 120, content.Labels, CurrentOption[Layer.Beard]), 1);
            ...
            // Shirt Color
            AddCustomColorPicker(489, 191, null, Layer.Shirt, 3000440, 10, 20);

            // Pants Color
            if (race != RaceType.GARGOYLE)
                AddCustomColorPicker(489, 241, null, Layer.Pants, 3000441, 10, 20);

            // Hair
            pallet = CharacterCreationValues.GetHairPallet(race);
            AddCustomColorPicker(489, 291, pallet, Layer.Hair, race == RaceType.GARGOYLE ? 1112322 : 3000184, 8, pallet.Length >> 3);

            if (!isFemale && race != RaceType.ELF)
            {
                // Facial
                pallet = CharacterCreationValues.GetHairPallet(race);
                AddCustomColorPicker(489, 341, pallet, Layer.Beard, race == RaceType.GARGOYLE ? 1112512 : 3000446, 8, pallet.Length >> 3);
            }
        ...
        private class CustomColorPicker : Control
        {
            ...
            public CustomColorPicker(Layer layer, int label, ushort[] pallet, int rows, int columns)
            {
                ...
                Add(new Label(ClilocLoader.Instance.GetString(label), true, 999, font: 0xFF)
                {
                    X = 0,
                    Y = 0
                });

                Add(_colorPicker = new ColorPickerBox(1, 20, 1, 1, 121, 23, pallet));
