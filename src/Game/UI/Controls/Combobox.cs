...
namespace ClassicUO.Game.UI.Controls
{
    internal class Combobox : Control
    {
        ...
        public Combobox(int x, int y, int width, string[] items, int selected = -1, int maxHeight = 0, bool showArrow = true, string emptyString = "", byte font = 1)
        {
            X = x;
            Y = y;
            Width = width;
            ...
            Add(_label = new Label(initialText, true, 0x0453, font: _font)
            {
                X = 2, Y = 5
            });
        ...
        private class ComboboxContextMenu : Control
        {
            ...
            public ComboboxContextMenu(Combobox box, string[] items, int minWidth, int maxHeight)
            {
                ...
                for (int i = 0; i < items.Length; i++)
                {
                    ...
                    var label = new HoveredLabel(item, true, 0x0453, 0x0453, 0x0453, font: _box._font)
                    {
                        X = 2,
                        Y = index * 15,
                        Tag = index,
