...
namespace ClassicUO.Game.UI.Controls
{
    internal class MacroControl : Control
    {
        ...
        public MacroControl(string name)
        {
            ...
            Add(new NiceButton(0, _hotkeyBox.Height + 3, 170, 25, ButtonAction.Activate, LanguageManager.Current.UI_AddMacroButton, 0, IO.Resources.TEXT_ALIGN_TYPE.TS_LEFT) { ButtonParameter = 2, IsSelectable = false });

            Add(new NiceButton(0, _hotkeyBox.Height + 30, 50, 25, ButtonAction.Activate, LanguageManager.Current.UI_Add) {IsSelectable = false});
            Add(new NiceButton(52, _hotkeyBox.Height + 30, 50, 25, ButtonAction.Activate, LanguageManager.Current.UI_Remove) {ButtonParameter = 1, IsSelectable = false});
        ...
        {
            _collection.Macro.Repeat = _checkbox.IsChecked;
        }
        private void Delaychanged(object sender, EventArgs e)
        {
            _collection.Macro.Delay = _delaybar.Value;
        }
        private void BoxOnHotkeyChanged(object sender, EventArgs e)
        {
            ...
            if (_hotkeyBox.Key != SDL.SDL_Keycode.SDLK_UNKNOWN)
            {
                ...
                if (macro != null)
                {
                    ...
                    SetupKeyByDefault();
                    UIManager.Add(new MessageBoxGump(250, 150, LanguageManager.Current.UI_Macro_Exists, null));
                    return;
                }
            }
            else 
