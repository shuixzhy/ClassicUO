...
namespace ClassicUO.Network
{
            ...
            if (type > 0 && p.Position + 1 <= p.Length)
            {
                ...
                if (mobile == World.Player)
                {
                    ...
                    if (World.Player.Strength != 0)
                    {
                        ...
                        if (deltaStr != 0)
                            GameActions.Print($"Your strength has changed by {deltaStr}.  It is now {str}", 0x0170, Game.MessageType.System, 3, false);

                        if (deltaDex != 0)
                            GameActions.Print($"Your dexterity has changed by {deltaDex}.  It is now {dex}", 0x0170, Game.MessageType.System, 3, false);

                        if (deltaInt != 0)
                            GameActions.Print($"Your intelligence has changed by {deltaInt}.  It is now {intell}", 0x0170, Game.MessageType.System, 3, false);
                    }
 ...
 //           if (graphic == 0x2006 && ProfileManager.Current.AutoOpenCorpses) World.Player.TryOpenCorpses();
        ...
        private static void Talk(Packet p)
        {
            ...
            ushort graphic = p.ReadUShort();
            Game.MessageType type = (Game.MessageType) p.ReadByte();
            ushort hue = p.ReadUShort();
            ushort font = p.ReadUShort();
            string name = p.ReadASCII(30);
            string text = p.ReadASCII();

            if (serial == 0 && graphic == 0 && type == Game.MessageType.Regular && font == 0xFFFF && hue == 0xFFFF && name.StartsWith("SYSTEM"))
            {
            ...
            if (!(type == Game.MessageType.System || serial == 0xFFFF_FFFF || serial == 0 || (name.ToLower() == "system" && entity == null)))
            {
                if (entity != null)
                {
        ...
        private static void DenyMoveItem(Packet p)
        {
            ...
            if (code < 5) Chat.HandleMessage(null, ServerErrorMessages.GetError(p.ID, code), string.Empty, 1001, Game.MessageType.System, 3);
        }
